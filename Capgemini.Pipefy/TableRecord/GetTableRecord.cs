...
namespace Capgemini.Pipefy.TableRecord
{
    ...
    {
        private const string GetTableRecordQuery = "query {{ table_record(id: {0}){{ created_at created_by {{ id name email }} due_date finished_at id status {{ id name }} record_fields {{ name value }} table {{ id name }} title updated_at url }} }}";
